...
import { canManageSettings, getRoleLabel } from "@/lib/access";
                      ...
                      <div className="flex flex-col gap-1">
                        {member.status === "ACTIVE" && (
                          <Button
                            ...
                            onClick={() => handleAction("disable", member.id)}
                          >
                            Desactivar
                          </Button>
                        )}
                        {member.status === "ACTIVE" &&
                          member.role === "CASHIER" && (
                            <Button
                              ...
                              size="sm"
                              className="w-full justify-start"
                              onClick={() => handleAction("revoke", member.id)}
                            >
                              Cerrar Sesión
                            </Button>
                          )}
                        {member.status === "DISABLED" && (
                          <Button
                            ...
                            size="sm"
                            className="w-full justify-start"
                            onClick={() => handleAction("enable", member.id)}
                          >
                            Activar
                          </Button>
                        )}
                        {member.status === "PENDING" && (
                          <Button
                            ...
                            size="sm"
                            className="w-full justify-start"
                            onClick={() => handleAction("cancel", member.id)}
                          >
                            Cancelar Invitación
                          </Button>
                        )}
